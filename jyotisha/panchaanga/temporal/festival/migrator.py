...
def migrate_db(old_db_file, only_descriptions=False):
  ...
  output_dir = os.path.join(CODE_ROOT, 'panchaanga/temporal/festival/data')
  # import shutil
...
def write_event_README(event, event_file_name):
  with open(event_file_name) as event_data:
    readme_file_name = os.path.join(os.path.dirname(event_file_name), 'data/README.md')
...
def migrate_relative_db():
  ...
  for old_style_event in old_style_events:
    event = HinduCalendarEvent.from_old_style_event(old_style_event=old_style_event)
    logging.debug(str(event))
    event_file_name = event.get_storage_file_name(base_dir=os.path.join(CODE_ROOT, 'panchaanga/temporal/festival/data'))
    logging.debug(event_file_name)
    event.dump_to_file(filename=event_file_name)
    write_event_README(event, event_file_name)
...
if __name__ == '__main__':
  ...
  shutil.rmtree(os.path.join(CODE_ROOT, 'panchaanga/temporal/festival/data'))
  migrate_db(os.path.join(CODE_ROOT, 'panchaanga/temporal/festival/data/legacy/festival_rules.json'))
  migrate_db(os.path.join(CODE_ROOT, 'panchaanga/temporal/festival/data/legacy/festival_rules_desc_only.json'),
             only_descriptions=True)
  migrate_relative_db()
  pass
