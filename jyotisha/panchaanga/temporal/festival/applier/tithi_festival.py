...
class TithiFestivalAssigner(FestivalAssigner):
  ...
  def assign_vishesha_saptami(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      # SPECIAL SAPTAMIs
      if self.panchaanga.weekday[d] == 0 and (self.panchaanga.daily_panchaangas[d].tithi_at_sunrise % 15) == 7:
        ...
        if yati_ekadashi_fday == smaarta_ekadashi_fday == vaishnava_ekadashi_fday is None:
          ...
          if ekadashi_paksha == 'shukla':
            ...
            harivasara_end = brentq(
              ...
              self.panchaanga.daily_panchaangas[smaarta_ekadashi_fday].jd_sunrise - 2,
              self.panchaanga.daily_panchaangas[smaarta_ekadashi_fday].jd_sunrise + 2)
          [_y, _m, _d, _t] = time.jd_to_utc_gregorian(harivasara_end + (tz_off / 24.0))
          hariv_end_time = Hour(
            time.jd_to_utc_gregorian(harivasara_end + (tz_off / 24.0))[3]).toString(
  ...
  def assign_amavasya_yoga(self, debug_festivals=False):
    ...
    for d in range(1, self.panchaanga.duration + 1):
      ...
      # SOMAMAVASYA
      if self.panchaanga.daily_panchaangas[d].tithi_at_sunrise == 30 and self.panchaanga.weekday[d] == 1:
      ...
      tithi_sunset = NakshatraDivision(self.panchaanga.daily_panchaangas[d].jd_sunset, ayanamsha_id=self.panchaanga.ayanamsha_id).get_anga(
        zodiac.AngaType.TITHI)
      if self.panchaanga.weekday[d] == 0 and (self.panchaanga.daily_panchaangas[d].tithi_at_sunrise == 30 or tithi_sunset == 30):
