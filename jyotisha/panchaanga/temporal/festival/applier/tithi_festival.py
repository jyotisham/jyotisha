...
class TithiFestivalAssigner(FestivalAssigner):
  ...
  def assign_chaturthi_vratam(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      if self.daily_panchaangas[d].angas.tithi_at_sunrise == 18 or self.daily_panchaangas[d].angas.tithi_at_sunrise == 19:
        ...
        _m = self.daily_panchaangas[d].lunar_month
        if floor(_m) != _m:
          _m = 13  # Adhika masa
        ...
        if tithi_moonrise == 19:
          ...
          if tithi_moonrise_yest != 19:
            ...
            self.daily_panchaangas[d]. festivals.append(FestivalInstance(name=chaturthi_name + 'saGkaTahara-caturthI-vratam'))
            # shravana krishna chaturthi
            if self.daily_panchaangas[d].lunar_month == 5:
              chaturthi_name = '%s%s' % ('aGgArakI~' if self.daily_panchaangas[d].date.get_weekday() == 2 else '', chaturthi_name)
              self.daily_panchaangas[d].festivals[-1] = FestivalInstance(name=chaturthi_name + 'mahAsaGkaTahara-caturthI-vratam')
  ...
  def assign_shasthi_vratam(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      if self.daily_panchaangas[d].lunar_month == 8:
        festival_name = 'skanda' + festival_name
      elif self.daily_panchaangas[d].lunar_month == 4:
        festival_name = 'kumAra-' + festival_name
      elif self.daily_panchaangas[d].lunar_month == 6:
        festival_name = 'SaSThIdEvI-' + festival_name
      elif self.daily_panchaangas[d].lunar_month == 9:
        festival_name = 'subrahmaNya-' + festival_name

      if self.daily_panchaangas[d].angas.tithi_at_sunrise == 5 or self.daily_panchaangas[d].angas.tithi_at_sunrise == 6:
  ...
  def assign_vishesha_saptami(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      # SPECIAL SAPTAMIs
      if self.daily_panchaangas[d].date.get_weekday() == 0 and (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 7:
        festival_name = 'bhAnusaptamI'
        if self.daily_panchaangas[d].angas.tithi_at_sunrise == 7:
          festival_name = 'vijayA' + '~' + festival_name
        if self.daily_panchaangas[d].nakshatra_at_sunrise == 27:
      ...
      if NakshatraDivision(self.daily_panchaangas[d].jd_sunrise, ayanaamsha_id=self.ayanaamsha_id).get_anga(
          ...
          self.daily_panchaangas[d].angas.tithi_at_sunrise == 7:
  ...
  def assign_ekaadashii_vratam(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      if (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 10 or (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 11:
        yati_ekaadashii_fday = smaarta_ekaadashii_fday = vaishnava_ekaadashii_fday = None
        ekaadashii_tithi_days = [x.angas.tithi_at_sunrise % 15 for x in self.daily_panchaangas[d:d + 3]]
        if self.daily_panchaangas[d].angas.tithi_at_sunrise > 15:
        ...
        if yati_ekaadashii_fday == smaarta_ekaadashii_fday == vaishnava_ekaadashii_fday is None:
          ...
          if smaarta_ekaadashii_fday == vaishnava_ekaadashii_fday:
            ...
            self.add_festival(
              'sarva-' + names.get_ekaadashii_name(ekaadashii_paksha, self.daily_panchaangas[d].lunar_month),
            ...
            self.add_festival(
              'smArta-' + names.get_ekaadashii_name(ekaadashii_paksha, self.daily_panchaangas[d].lunar_month),
              smaarta_ekaadashii_fday, debug_festivals)
            self.add_festival(
              'vaiSNava-' + names.get_ekaadashii_name(ekaadashii_paksha, self.daily_panchaangas[d].lunar_month),
          ...
          self.add_festival('smArta-' + names.get_ekaadashii_name(ekaadashii_paksha,
                                                                self.daily_panchaangas[d].lunar_month) + ' (gRhastha)',
                            smaarta_ekaadashii_fday, debug_festivals)
        ...
        if yati_ekaadashii_fday == smaarta_ekaadashii_fday == vaishnava_ekaadashii_fday is None:
          ...
          self.daily_panchaangas[int(fday_hv)].festivals.append(
  ...
  def assign_mahaadvaadashii(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      [y, m, dt, t] = time.jd_to_utc_gregorian(self.panchaanga.jd_start + d - 1).to_date_fractional_hour_tuple()
      # 8 MAHA DWADASHIS
      if (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 11 and (self.daily_panchaangas[d + 1].angas.tithi_at_sunrise % 15) == 11:
        self.add_festival('unmIlanI~mahAdvAdazI', d + 1, debug_festivals)

      if (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12 and (self.daily_panchaangas[d + 1].angas.tithi_at_sunrise % 15) == 12:
        self.add_festival('vyaJjulI~mahAdvAdazI', d, debug_festivals)

      if (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 11 and (self.daily_panchaangas[d + 1].angas.tithi_at_sunrise % 15) == 13:
        self.add_festival('trisparzA~mahAdvAdazI', d, debug_festivals)

      if (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 0 and (self.daily_panchaangas[d + 1].angas.tithi_at_sunrise % 15) == 0:
        # Might miss out on those parva days right after Dec 31!
        if (d - 3) > 0:
          self.add_festival('pakSavardhinI~mahAdvAdazI', d - 3, debug_festivals)

      if self.daily_panchaangas[d].nakshatra_at_sunrise == 4 and (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12:
        self.add_festival('pApanAzinI~mahAdvAdazI', d, debug_festivals)

      if self.daily_panchaangas[d].nakshatra_at_sunrise == 7 and (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12:
        self.add_festival('jayantI~mahAdvAdazI', d, debug_festivals)

      if self.daily_panchaangas[d].nakshatra_at_sunrise == 8 and (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12:
        self.add_festival('jayA~mahAdvAdazI', d, debug_festivals)

      if self.daily_panchaangas[d].nakshatra_at_sunrise == 8 and (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12 and self.daily_panchaangas[d].lunar_month == 12:
      ...
      if (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12:
        if self.daily_panchaangas[d].nakshatra_at_sunrise in [21, 22, 23]:
          # We have a dvaadashii near shravana, check for Shravana sparsha
          for td in [x.angas.tithis_with_ends for x in self.daily_panchaangas[d:d + 2]]:
            (t12, t12_end) = td[0]
            if t12_end is None:
              continue
            if (t12 % 15) == 11:
              if NakshatraDivision(t12_end, ayanaamsha_id=self.ayanaamsha_id).get_anga(
                ...
                elif (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12:
                  self.add_festival('vijayA/zravaNa-mahAdvAdazI', d, debug_festivals)
                elif (self.daily_panchaangas[d + 1].angas.tithi_at_sunrise % 15) == 12:
      ...
      if self.daily_panchaangas[d].nakshatra_at_sunrise == 22 and (self.daily_panchaangas[d].angas.tithi_at_sunrise % 15) == 12:
        self.add_festival('vijayA/zravaNa-mahAdvAdazI', d, debug_festivals)
  ...
  def assign_pradosha_vratam(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      # PRADOSHA Vratam
      pref = ''
      if self.daily_panchaangas[d].angas.tithi_at_sunrise in (12, 13, 27, 28):
  ...
  def assign_amavasya_yoga(self, debug_festivals=False):
    if 'amAvAsyA' not in self.panchaanga.festival_id_to_instance:
      ...
      for d in ama_days:
        ...
        if self.daily_panchaangas[d].lunar_month == 6:
          pref = '(%s) mahAlaya ' % (
            names.get_chandra_masa(self.daily_panchaangas[d].lunar_month, names.NAMES, 'hk', visarga=False))
          ...
          pref = names.get_chandra_masa(self.daily_panchaangas[d].lunar_month, names.NAMES, 'hk',
                                        visarga=False) + '-'
        ...
        suff = ''
        # Assign
        if 23 in ama_nakshatra_today and self.daily_panchaangas[d].lunar_month == 10:
    ...
    for d in range(1, self.panchaanga.duration + 1):
      ...
      # SOMAMAVASYA
      if self.daily_panchaangas[d].angas.tithi_at_sunrise == 30 and self.daily_panchaangas[d].date.get_weekday() == 1:
      ...
      if self.daily_panchaangas[d].date.get_weekday() == 0 and (self.daily_panchaangas[d].angas.tithi_at_sunrise == 30 or tithi_sunset == 30):
        # AMAVASYA on a Sunday
        if (self.daily_panchaangas[d].nakshatra_at_sunrise in [1, 6, 9, 22, 23] and self.daily_panchaangas[d].angas.tithi_at_sunrise == 30) or \
  ...
  def assign_chandra_darshanam(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      [y, m, dt, t] = time.jd_to_utc_gregorian(self.panchaanga.jd_start + d - 1).to_date_fractional_hour_tuple()
      # Chandra Darshanam
      if self.daily_panchaangas[d].angas.tithi_at_sunrise == 1 or self.daily_panchaangas[d].angas.tithi_at_sunrise == 2:
        ...
        if tithi_sunset <= 2:
          if tithi_sunset == 1:
            self.daily_panchaangas[d + 1]. festivals.append(FestivalInstance(name='candra-darzanam'))
          else:
            self.daily_panchaangas[d]. festivals.append(FestivalInstance(name='candra-darzanam'))
  ...
  def assign_vishesha_trayodashi(self, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      [y, m, dt, t] = time.jd_to_utc_gregorian(self.panchaanga.jd_start + d - 1).to_date_fractional_hour_tuple()
      # VARUNI TRAYODASHI
      if self.daily_panchaangas[d].lunar_month == 12 and self.daily_panchaangas[d].angas.tithi_at_sunrise == 28:
        if NakshatraDivision(self.daily_panchaangas[d].jd_sunrise, ayanaamsha_id=self.ayanaamsha_id).get_anga(
