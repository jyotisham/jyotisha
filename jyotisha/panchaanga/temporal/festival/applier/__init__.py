...
class FestivalAssigner(PanchaangaApplier):
  ...
  def assign_festivals_from_rules(self, festival_rules, debug_festivals=False):
    for d in range(1, self.panchaanga.duration + 1):
      ...
      for festival_name in festival_rules:
        if festival_rules[festival_name].timing.month_type is not None:
        ...
        if anga_type == 'tithi' and month_type == 'lunar_month' and angam_num == 1:
          ...
          if self.daily_panchaangas[d].sunrise_day_angas.tithi_at_sunrise == 30 and self.daily_panchaangas[d + 1].sunrise_day_angas.tithi_at_sunrise == 2 and \
              self.daily_panchaangas[d + 1].lunar_month_sunrise == month_num:
        ...
        if anga_type == 'day' and month_type == 'sidereal_solar_month' and self.daily_panchaangas[d].solar_sidereal_date_sunset.month == month_num:
          if self.daily_panchaangas[d].solar_sidereal_date_sunset.day == angam_num:
            if kaala == 'arunodaya':
              ...
              self.add_festival(festival_name, d)
        elif (month_type == 'lunar_month' and ((self.daily_panchaangas[d].lunar_month_sunrise == month_num or month_num == 0) or (
            (self.daily_panchaangas[d + 1].lunar_month_sunrise == month_num and angam_num == 1)))) or \
          ...
          if anga_type == 'tithi':
            angam_sunrise = [d.sunrise_day_angas.tithi_at_sunrise for d in self.daily_panchaangas]
            angam_data = [d.sunrise_day_angas.tithis_with_ends for d in self.daily_panchaangas]
            get_angam_func = lambda x: temporal.tithi.get_tithi(x)
            num_angams = 30
          elif anga_type == 'nakshatra':
            angam_sunrise = [d.sunrise_day_angas.nakshatra_at_sunrise for d in self.daily_panchaangas]
            angam_data = [d.sunrise_day_angas.nakshatras_with_ends for d in self.daily_panchaangas]
            get_angam_func = lambda x: NakshatraDivision(x, ayanaamsha_id=self.ayanaamsha_id).get_nakshatra()
            num_angams = 27
          elif anga_type == 'yoga':
            angam_sunrise = [d.sunrise_day_angas.yoga_at_sunrise for d in self.daily_panchaangas]
            angam_data = [d.sunrise_day_angas.yogas_with_ends for d in self.daily_panchaangas]
          ...
          if angam_sunrise[d] == prev_angam or angam_sunrise[d] == angam_num:
            ...
            if priority == 'paraviddha':
              ...
              if angams in ([p, p, q, q], [p, q, q, q], [q, q, q, q], [p, p, q, r], [p, p, r, r]):
                ...
                anga_span = angam_data[d][0]
                (angam, angam_end) = (anga_span.name, anga_span.jd_end)
                vyapti_1 = max(t_end_d - angam_end, 0)
                vyapti_2 = max(angam_end - t_start_d1, 0)
                for anga_span in angam_data[d + 1]:
                  angam_end = anga_span.jd_end
          ...
          if fday is not None:
            if (month_type == 'lunar_month' and ((self.daily_panchaangas[d].lunar_month_sunrise == month_num or month_num == 0) or (
                (self.daily_panchaangas[d + 1].lunar_month_sunrise == month_num and angam_num == 1)))) or \
              ...
              if month_type == 'lunar_month' and angam_num == 1 and self.daily_panchaangas[
                fday + 1].lunar_month_sunrise != month_num:
                continue
  ...
  def assign_festival_numbers(self, festival_rules, debug_festivals=False):
    ...
    for d in range(1, self.panchaanga.duration + 1):
      ...
      if self.daily_panchaangas[d].lunar_month_sunrise == 1 and self.daily_panchaangas[d - 1].lunar_month_sunrise != 1:
        lunar_y_start_d.append(d)

    period_start_year = self.panchaanga.start_date.year
    for festival_name in festival_rules:
      if festival_name in self.panchaanga.festival_id_to_instance and festival_rules[festival_name].timing.year_start is not None:
        ...
        for assigned_day in self.panchaanga.festival_id_to_instance[festival_name].days:
          ...
          if fest_num <= 0:
            ...
            if festival_name_updated in self.panchaanga.festival_id_to_instance:
              ...
              self.panchaanga.festival_id_to_instance[festival_name_updated] = festival.FestivalInstance(
                id=festival_name_updated, days=[assigned_day])
            else:
              self.panchaanga.festival_id_to_instance[festival_name_updated] = festival.FestivalInstance(
                id=festival_name_updated, days=[assigned_day])
...
class MiscFestivalAssigner(FestivalAssigner):
  ...
  def assign_relative_festivals(self):
    ...
    if 'yajurvEda-upAkarma' not in self.panchaanga.festival_id_to_instance:
      ...
      self.panchaanga.festival_id_to_instance['varalakSmI-vratam'] = festival.FestivalInstance(id='varalakSmI-vratam', days=[])
    ...
    for festival_name in relative_festival_rules:
      ...
      if rel_festival_name not in self.panchaanga.festival_id_to_instance:
        ...
        if matched_festivals == []:
          ...
          self.panchaanga.festival_id_to_instance[festival_name] = festival.FestivalInstance(id=festival_name, days=[x + offset for x in self.panchaanga.festival_id_to_instance[matched_festivals[0]].days])
      else:
        self.panchaanga.festival_id_to_instance[festival_name] = festival.FestivalInstance(id=festival_name, days=[x + offset for x in self.panchaanga.festival_id_to_instance[rel_festival_name].days])

    for _, fest in self.panchaanga.festival_id_to_instance.items():
      for fest_day in fest.days:
        fest_day_str = fest_day.get_date_str()
        if fest_day_str in self.panchaanga.date_str_to_panchaanga:
          self.panchaanga.date_str_to_panchaanga[fest_day_str].festivals.append(fest)
