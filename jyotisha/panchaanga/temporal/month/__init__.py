...
class SiderialSolarBasedAssigner(LunarMonthAssigner):
  def assign(self):
    ...
    last_new_moon = AngaSpan.find(
      ...
      zodiac.AngaType.TITHI, 30, ayanaamsha_id=self.panchaanga.ayanaamsha_id)
    this_new_moon = AngaSpan.find(
      last_new_moon.jd_start + 24, last_new_moon.jd_start + 32,
      zodiac.AngaType.TITHI, 30, ayanaamsha_id=self.panchaanga.ayanaamsha_id)

    # Check if current mAsa is adhika here
    is_adhika = NakshatraDivision(last_new_moon.jd_end, ayanaamsha_id=self.panchaanga.ayanaamsha_id).get_solar_raashi() == \
                NakshatraDivision(this_new_moon.jd_end, ayanaamsha_id=self.panchaanga.ayanaamsha_id).get_solar_raashi()
    ...
    while last_new_moon.jd_start < self.panchaanga.jd_start + self.panchaanga.duration + 1:
      ...
      is_adhika = NakshatraDivision(this_new_moon.jd_end, ayanaamsha_id=self.panchaanga.ayanaamsha_id).get_solar_raashi() == \
                  NakshatraDivision(next_new_moon.jd_end, ayanaamsha_id=self.panchaanga.ayanaamsha_id).get_solar_raashi()
