...
class NakshatraAssigner(PanchaangaApplier):
  def calc_nakshatra_tyaajya(self, debug=False):
    ...
    for d in range(1, self.panchaanga.duration + 1):
      ...
      t_start = self.panchaanga.daily_panchaangas[d - 1].nakshatras_with_ends[-1][1]
      if t_start is not None:
        n, t_end = self.panchaanga.daily_panchaangas[d].nakshatras_with_ends[0]
        if t_end is None:
          t_end = self.panchaanga.daily_panchaangas[d + 1].nakshatras_with_ends[0][1]
      ...
      if len(self.panchaanga.daily_panchaangas[d].nakshatras_with_ends) == 2:
        t_start = t_end
        n2, t_end = self.panchaanga.daily_panchaangas[d].nakshatras_with_ends[1]
