...
def init_names_auto(fname=os.path.join(os.path.dirname(os.path.dirname(__file__)), 'festival/data/period_names/misc_names.toml')):
  ...
  with open(fname) as f:
    ...
    for dictionary in names_dict:
      if dictionary in ("SHUULAM", "SA_TO_TAMIL", "ARAB_MONTH_NAMES", "TIPU_ABJAD_MONTH_NAMES", "TIPU_ABTATH_MONTH_NAMES", "SIDEREAL_SOLAR_MONTH_NAMES") or not isinstance(names_dict[dictionary]['sa'], list):
        continue
