...
def to_table_dict(panchaanga, script=sanscript.DEVANAGARI):
  ...
  for daily_panchaanga in panchaanga.daily_panchaangas_sorted(skip_padding_days=True):
    ...
    day_dict["sidereal_solar_month"] = daily_panchaanga.get_month_str(month_type=RulesRepo.SIDEREAL_SOLAR_MONTH_DIR, script=script)
    day_dict["tithis"] = daily_panchaanga.sunrise_day_angas.get_anga_data_md(anga_type=AngaType.TITHI, script=script, reference_jd=daily_panchaanga.julian_day_start)
    day_dict["vaara"] = names.NAMES['VARA_NAMES']['sa'][script][daily_panchaanga.date.get_weekday()]
    day_dict["lunar_nakshatras"] = daily_panchaanga.sunrise_day_angas.get_anga_data_md(anga_type=AngaType.NAKSHATRA, script=script, reference_jd=daily_panchaanga.julian_day_start)
