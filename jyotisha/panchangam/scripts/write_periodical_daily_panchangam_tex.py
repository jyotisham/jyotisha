...
def writeDailyTeX(panchangam, template_file, compute_lagnams=True, output_stream=None):
    ...
    samvatsara_names = [jyotisha.names.NAMES['SAMVATSARA_NAMES'][panchangam.script][(_x + 12) % 60 + 1] for _x in list(range(kali_year_start, kali_year_end + 1))]
    ...
    for d in range(1, panchangam.duration + 1):
        ...
        for rashi_ID, rashi_end_jd in panchangam.rashi_data[d]:
            rashi = jyotisha.names.NAMES['RASHI_NAMES'][panchangam.script][rashi_ID]
            if rashi_end_jd is None:
                chandrashtama_rashi_data_str = '\\mbox{%s%s}' % (chandrashtama_rashi_data_str, jyotisha.names.NAMES['RASHI_NAMES'][panchangam.script][((rashi_ID - 8) % 12) + 1])
            else:
                chandrashtama_rashi_data_str = '\\mbox{%s%s\To{}\\textsf{%s}}' % (chandrashtama_rashi_data_str, jyotisha.names.NAMES['RASHI_NAMES'][panchangam.script][((rashi_ID - 8) % 12) + 1], jyotisha.panchangam.temporal.hour.Hour(24 * (rashi_end_jd - jd)).toString(format=panchangam.fmt))
