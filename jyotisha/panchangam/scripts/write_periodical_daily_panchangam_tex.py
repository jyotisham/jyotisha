...
def writeDailyTeX(panchangam, template_file, compute_lagnams=True, output_stream=None):
  ...
  samvatsara_names = [jyotisha.names.NAMES['SAMVATSARA_NAMES'][panchangam.script][(_x + 12) % 60 + 1] for _x in
  ...
  print('{\\sffamily \\fontsize{20}{20}\\selectfont  %4d-%02d-%02d–%4d-%02d-%02d\\\\[0.5cm]}'
        % (panchangam.start_date[0], panchangam.start_date[1], panchangam.start_date[2], panchangam.end_date[0],
           panchangam.end_date[1], panchangam.end_date[2]), file=output_stream)
  ...
  for d in range(1, panchangam.duration + 1):
        ...
        tithi_data_str = '%s\\mbox{%s\\To{}\\textsf{%s (%s)}}' % \
                         (tithi_data_str, tithi,
                          jyotisha.panchangam.temporal.hour.Hour(
                            24 * (tithi_end_jd - panchangam.jd_sunrise[d])).toString(format='gg-pp'),
    ...
    for nakshatram_ID, nakshatram_end_jd in panchangam.nakshatram_data[d]:
      ...
      if nakshatram_end_jd is None:
        ...
        nakshatram_data_str = '%s\\mbox{%s\\To{}\\textsf{%s (%s)}}' % \
                              (nakshatram_data_str, nakshatram,
                               jyotisha.panchangam.temporal.hour.Hour(
                                 24 * (nakshatram_end_jd - panchangam.jd_sunrise[d])).toString(format='gg-pp'),
                               jyotisha.panchangam.temporal.hour.Hour(24 * (nakshatram_end_jd - jd)).toString(
                                 format=panchangam.fmt))
    amritadi_yoga_str = '/'.join(
      [jyotisha.custom_transliteration.tr(_x, panchangam.script) for _x in amritadi_yoga_list])
    ...
    if panchangam.tyajyam_data[d] == []:
      ...
      for td in panchangam.tyajyam_data[d]:
        tyajyam_data_str += '%s--%s\\hspace{2ex}' % (
          jyotisha.panchangam.temporal.hour.Hour(24 * (td[0] - jd)).toString(format=panchangam.fmt),
          jyotisha.panchangam.temporal.hour.Hour(24 * (td[1] - jd)).toString(format=panchangam.fmt))
    ...
    for rashi_ID, rashi_end_jd in panchangam.rashi_data[d]:
      ...
      rashi = jyotisha.names.NAMES['RASHI_NAMES'][panchangam.script][rashi_ID]
      if rashi_end_jd is None:
        rashi_data_str = '%s\\mbox{%s}' % (rashi_data_str, rashi)
      else:
        rashi_data_str = '%s\\mbox{%s\\RIGHTarrow\\textsf{%s}}' % \
                         (rashi_data_str, rashi,
                          jyotisha.panchangam.temporal.hour.Hour(24 * (rashi_end_jd - jd)).toString(
                            format=panchangam.fmt))
    ...
    for rashi_ID, rashi_end_jd in panchangam.rashi_data[d]:
      ...
      if rashi_end_jd is None:
        ...
        chandrashtama_rashi_data_str, jyotisha.names.NAMES['RASHI_NAMES'][panchangam.script][((rashi_ID - 8) % 12) + 1])
      else:
    ...
    jyotisha.custom_transliteration.tr('zUlam', panchangam.script),
    ...
    for yoga_ID, yoga_end_jd in panchangam.yoga_data[d]:
      ...
      if yoga_end_jd is None:
        ...
        yoga_data_str = '%s\\mbox{%s\\To{}\\textsf{%s (%s)}}\\hspace{1ex}' % \
                        (yoga_data_str, yoga,
                         jyotisha.panchangam.temporal.hour.Hour(24 * (yoga_end_jd - panchangam.jd_sunrise[d])).toString(
                           format='gg-pp'),
    ...
    for numKaranam, (karanam_ID, karanam_end_jd) in enumerate(panchangam.karanam_data[d]):
      ...
      if karanam_end_jd is None:
        ...
        karanam_data_str = '%s\\mbox{%s\\To{}\\textsf{%s (%s)}}\\hspace{1ex}' % \
                           (karanam_data_str, karanam,
                            jyotisha.panchangam.temporal.hour.Hour(
                              24 * (karanam_end_jd - panchangam.jd_sunrise[d])).toString(format='gg-pp'),
    ...
    if panchangam.shraaddha_tithi[d] == [None]:
      ...
      if panchangam.shraaddha_tithi[d][0] == 0:
        ...
        t1 = jyotisha.names.NAMES['TITHI_NAMES'][panchangam.script][panchangam.shraaddha_tithi[d][0]]
        if len(panchangam.shraaddha_tithi[d]) == 2:
          t2 = jyotisha.names.NAMES['TITHI_NAMES'][panchangam.script][panchangam.shraaddha_tithi[d][1]]
          stithi_data_str = '%s/%s (%s)' % \
                            (t1.split('-')[-1], t2.split('-')[-1],
                             jyotisha.custom_transliteration.tr('tithidvayam', panchangam.script))
        else:
    ...
    braahma_start = jyotisha.panchangam.temporal.hour.Hour(24 * (panchangam.kaalas[d]['braahma'][0] - jd)).toString(
      format=panchangam.fmt)
    pratahsandhya_start = jyotisha.panchangam.temporal.hour.Hour(
      24 * (panchangam.kaalas[d]['prAtaH sandhyA'][0] - jd)).toString(format=panchangam.fmt)
    pratahsandhya_end = jyotisha.panchangam.temporal.hour.Hour(
      24 * (panchangam.kaalas[d]['prAtaH sandhyA end'][0] - jd)).toString(format=panchangam.fmt)
    sangava = jyotisha.panchangam.temporal.hour.Hour(24 * (panchangam.kaalas[d]['saGgava'][0] - jd)).toString(
      format=panchangam.fmt)
    madhyaahna = jyotisha.panchangam.temporal.hour.Hour(24 * (panchangam.kaalas[d]['madhyAhna'][0] - jd)).toString(
      format=panchangam.fmt)
    madhyahnika_sandhya_start = jyotisha.panchangam.temporal.hour.Hour(
      24 * (panchangam.kaalas[d]['mAdhyAhnika sandhyA'][0] - jd)).toString(format=panchangam.fmt)
    madhyahnika_sandhya_end = jyotisha.panchangam.temporal.hour.Hour(
      24 * (panchangam.kaalas[d]['mAdhyAhnika sandhyA end'][0] - jd)).toString(format=panchangam.fmt)
    ...
    sayahna = jyotisha.panchangam.temporal.hour.Hour(24 * (panchangam.kaalas[d]['sAyAhna'][0] - jd)).toString(
      format=panchangam.fmt)
    sayamsandhya_start = jyotisha.panchangam.temporal.hour.Hour(
      24 * (panchangam.kaalas[d]['sAyaM sandhyA'][0] - jd)).toString(format=panchangam.fmt)
    sayamsandhya_end = jyotisha.panchangam.temporal.hour.Hour(
      24 * (panchangam.kaalas[d]['sAyaM sandhyA end'][0] - jd)).toString(format=panchangam.fmt)
    ratriyama1 = jyotisha.panchangam.temporal.hour.Hour(24 * (panchangam.kaalas[d]['rAtri yAma 1'][0] - jd)).toString(
      format=panchangam.fmt)
    shayana_time_end = jyotisha.panchangam.temporal.hour.Hour(24 * (panchangam.kaalas[d]['zayana'][0] - jd)).toString(
      format=panchangam.fmt)
    ...
    sar_data = '{%s}{%s}{%s}' % (yname,
                                 jyotisha.names.NAMES['AYANA_NAMES'][panchangam.script][panchangam.tropical_month[d]],
                                 jyotisha.names.NAMES['RTU_NAMES'][panchangam.script][panchangam.tropical_month[d]])
    ...
    print('{%s}' % '\\eventsep '.join(
      [jyotisha.custom_transliteration.tr(f, panchangam.script).replace('★', '$^\\star$') for f in
       sorted(set(panchangam.festivals[d]))]), file=output_stream)
